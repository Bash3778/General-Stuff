...
public class Lenscrafter : MonoBehaviour
{
    ...
    [SerializeField] GameObject generalConstructionContainer;
    [SerializeField] GameObject outerBound;
    [SerializeField] GameObject emptyObject;
    public GameObject surfaceTestButton;
    ...
    [SerializeField] Button helpSwitch;
    [SerializeField] Button backHelp;
    ...
    public static int totalPoints = 576;
    ...
    void Start()
    {
        ...
        saveAs[0].onClick.AddListener(delegate{saveAsFunction(0);});
        saveAs[1].onClick.AddListener(delegate{saveAsFunction(1);});
        saveAs[2].onClick.AddListener(delegate{saveAsFunction(2);});
        helpSwitch.onClick.AddListener(delegate{ helpOn(true); });
        backHelp.onClick.AddListener(delegate{ helpOn(false); });
        ...
        if (currentItem != null) {
            if (currentScript.current.GetComponent<ItemManager>().current != null && currentScript.current.GetComponent<ItemManager>().current.GetComponent<ItemManager>().on) {
                            ...
                            if (exp1.value == exp2.value) {
                                ...
                                */ 
                                ...
                                if (!inPlace) {
                                    ...
                                    obj.AddComponent<SurfaceRefrance>();
                                    obj.GetComponent<SurfaceRefrance>().refrence = tempScript;
                                    obj.transform.SetParent(generalConstructionContainer.transform);
                                    obj.tag = "lens";
                ...
                if (tempScript.lowers.Count > 0) {
                    ...
                    Rigidbody tempRig = tempScript.lowers[0].GetComponent<Rigidbody>();
                    tempRig.isKinematic = true;
                    tempRig.useGravity = false;
                    tempRig.constraints = RigidbodyConstraints.FreezeAll;
                }
            }
        }
    }
    ...
    void saveAsFunction (int lensType) {
        if (mainer.surfaceCounter[lensType] < mainer.surfaceSecondCount && currentItem != null) {
            GameObject obj1 = Instantiate(emptyObject);
            obj1.AddComponent<refractingSurface>();
            obj1.SetActive(true);
            refractingSurface surface = obj1.GetComponent<refractingSurface>();
            surface.angleI = new float[totalPoints];
            surface.angleJ = new float[totalPoints];
            surface.angleK = new float[totalPoints];
            surface.angleL = new float[totalPoints];
            surface.posX = new float[totalPoints];
            surface.posY = new float[totalPoints];
            surface.posZ = new float[totalPoints];
            surface.velX = new float[totalPoints];
            surface.velY = new float[totalPoints];
            surface.velZ = new float[totalPoints];
            surface.namer = currentScript;
            int countingSurface = 0;
            //circular coordinate y
            for (float i = 0; i < 360; i+= angleInterval) { // It must be < not <=
                //circular coordinate xz
                for (float j = 0; j < 360; j+= angleInterval) {
                    //rotational coordinate y
                    for (float k = -1 * degreeOpening / 2; k <= degreeOpening / 2; k += angleInterval2) { //degreeOpening / 2
                        ...
                        for (float l = -1 * degreeOpening / 2; l <= degreeOpening / 2; l+= angleInterval2) {
                            ...
                            surface.angleI[countingSurface] = i;
                            surface.angleJ[countingSurface] = j;
                            surface.angleK[countingSurface] = k;
                            surface.angleL[countingSurface] = l;
                            scripter.indexSurface = countingSurface;
                            countingSurface++;
                        }
                    }
                } 
            }
            GameObject obj3 = Instantiate(surfaceTestButton);
            obj3.SetActive(true);
            obj3.transform.SetParent(mainer.surfaceCanvas[lensType]);
            obj3.GetComponentInChildren<Text>().text = surface.namer.name;
            surface.mainbut = obj3;
            mainer.surfaceButtons[lensType, mainer.surfaceCounter[lensType]] =  obj3;
            mainer.surfaces[lensType, mainer.surfaceCounter[lensType]] = surface;
            mainer.surfaceCanvasUpdate(lensType);
            mainer.surfaceCounter[lensType]++;
        }
    }
    void helpOn (bool switchType) {
        if (switchType) {
    ...
    public void reorganize (List<GameObject> set, int layerType) {
        Text[] layerTags = new Text[] {itemTag, layerTag, planeTag};
        for (int i = 0; i < set.Count; i++) {
            set[i].transform.position = new Vector3(layerTags[layerType].transform.position.x, -1 * (i + 1 + 0.1f) * countingOffset + layerTags[layerType].transform.position.y);
            set[i].GetComponent<ItemManager>().setAnchors();
        }
    }
    void deleteFunc () {
        if (currentScript != null) {
            ItemManager currentDelete = currentScript;
            if (currentScript.current != null && currentScript.current.GetComponent<ItemManager>().on) {
                currentDelete = currentScript.current.GetComponent<ItemManager>();
                if (currentDelete.current != null && currentDelete.current.GetComponent<ItemManager>().on) {
                    currentDelete = currentDelete.current.GetComponent<ItemManager>();
                }
            }
            currentDelete.GetComponent<ItemManager>().deleteBelow();
            if (currentDelete.type > 0) {
                currentDelete.higher.lowers.Remove(currentDelete.gameObject);
                Destroy(currentDelete.gameObject);
                reorganize(currentDelete.higher.lowers, currentDelete.type);
            } else {
